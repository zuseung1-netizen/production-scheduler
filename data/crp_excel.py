...
class CRPManager:
    ...
    def create_template(self, save_path: str, shifts: List[int],
        ...
        """Generate a blank CRP Excel template (ShiftNo × Date)."""
        if not HAS_OPENPYXL:
            return False, "openpyxl not installed"
        try:
            wb = Workbook()
            ...
            headers = ["ShiftNo"] + [d.strftime("%Y-%m-%d") for d in dates]
            ...
            for ri, shift_no in enumerate(shifts, 2):
                ws.cell(row=ri, column=1, value=shift_no)
