...
class Scheduler:
    ...
    # ── MPS (Master Plan) Auto-Plan ───────────────────────────────────────────

    def mps_auto_plan(self, date_from: str, date_to: str,
                      progress_cb=None) -> Dict:
        """Rough planning at RoomType × Day level.
        Places MPS-level plans (plan_level='MPS', room_code='__MPS__', shift_no=0)
        without assigning specific Rooms or Shifts.
        Capacity is summed across all Rooms of the same RoomType × all open Shifts,
        then multiplied by rough_capacity_pct (default 80%).
        """
        crp_manager.refresh()
        self._reload_masters()

        try:
            rough_pct = float(ConfigRepo.get("rough_capacity_pct", "80")) / 100.0
        except (ValueError, TypeError):
            rough_pct = 0.80

        # Clear old unlocked MPS plans in window
        PlanRepo.delete_unlocked_mps(date_from, date_to)

        # Build day-level capacity map: {(date_str, room_type, process_name): cap_inner}
        day_cap = self._build_mps_day_cap(date_from, date_to, rough_pct)
        ...
        for idx, so in enumerate(campaign_sos):
            if progress_cb:
                progress_cb(idx, total, so)
            self._mps_plan_so(so, day_cap, date_from, date_to, report)

        return report

    def _build_mps_day_cap(self, date_from: str, date_to: str,
                            rough_pct: float) -> Dict[Tuple[str, str, str], float]:
        """Build {(date_str, room_type, process_name): aggregate_daily_capacity_inner}
        by summing all Rooms of same RoomType across all open Shifts on each day.
        Uses _compute_hc_distribution() for proper HC allocation."""
        _unavail = {
            (r["cal_date"], r["shift_no"], r["room_code"])
            for r in CalendarRepo.get_unavailable_slots(date_from, date_to)
        }

        day_cap: Dict[Tuple[str, str, str], float] = {}
        urgency = self._compute_urgency()
        cur, d1 = _date(date_from), _date(date_to)
        while cur <= d1:
            ds = _ds(cur)
            for shift in self.shifts:
                sno = shift["shift_no"]
                # Build active_rps for this date/shift (same as _build_slot_map)
                active_rps: List[Tuple[str, List[Dict]]] = []
                for room in self.rooms:
                    if (ds, sno, room) in _unavail:
                        continue
                ...
                hc_dist = self._compute_hc_distribution(ds, sno, active_rps, urgency)
                ...
                for room, rps in active_rps:
                    for rp in rps:
                        ...
                        if cap > 0:
                            rt = rp["room_type"]
                            pname = rp["process_name"]
                            key = (ds, rt, pname)
                            day_cap[key] = day_cap.get(key, 0.0) + cap
            # Apply rough_pct after summing all rooms+shifts for the day
            for key in list(day_cap.keys()):
                if key[0] == ds:
                    day_cap[key] *= rough_pct
            cur += timedelta(days=1)
        return day_cap
    ...
    def _mps_plan_so(self, so: Dict, day_cap: Dict[Tuple[str, str, str], float],
        ...
        sku = self.sku_map.get(sku_code)
        if not sku:
            report["routing_errors"].append({"so": so["so_number"],
                                              "reason": f"SKU {sku_code} not in master"})
            report["skipped"] += 1
            return

        uom = int(sku.get("uom") or 1)
        routing = ProcessRoutingRepo.for_entity("SKU", sku_code)
        if not routing:
            report["routing_errors"].append({"so": so["so_number"],
                                              "reason": f"No routing for {sku_code}"})
            report["skipped"] += 1
            return
    ...
    def auto_schedule(self, date_from: str, date_to: str,
                      progress_cb=None) -> Dict:
        """Detailed Room×Shift scheduling scoped to a short window.
        Promotes MPS plans into SCHEDULE plans within [date_from, date_to]
        using the existing auto_plan() logic. MPS plans outside this window
        are untouched."""
        return self.auto_plan(date_from, date_to, progress_cb=progress_cb)
