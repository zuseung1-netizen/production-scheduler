...
from data.repositories import (
    ...
    ActualRepo, ConfigRepo, MaterialDemandRepo, _now
)
...
class Scheduler:
    ...
    def auto_plan(self, date_from: str, date_to: str,
        ...
        self._reload_masters()
        # Always start with a clean slate so closed rooms / changed capacity
        # are fully reflected (locked plans are preserved).
        ...
        # 1. Plan SKU orders
        for idx, so in enumerate(open_sos):
            if progress_cb:
    ...
    def force_plan_so(self, so_number: str, sku_code: str, line_item: str,
        ...
        Plans from date_from to date_to regardless of due date / post_lead_days.
        Useful for late SOs that the regular auto-plan skips.
        """
        self._reload_masters()
    ...
    def _plan_so(self, so: Dict, slot_map: Dict[SlotKey, float],
        ...
        planned  = PlanRepo.planned_qty(so["so_number"], sku_code, so["line_item"])
        ...
        for si in range(len(steps) - 1, -1, -1):
            ...
            if si == len(steps) - 1:
                upper_d, upper_s = d1, max(sh["shift_no"] for sh in self.shifts)
            else:
            ...
            for ds, sno, room_code, rp in candidates:
                if step_rem <= 0:
                    break
                key: SlotKey = (ds, room_code, step["process_name"], sno)
    ...
    def _plan_all_materials(self, slot_map: Dict[SlotKey, float],
        ...
        sku_plans = PlanRepo.all(entity_type="SKU")
    ...
    def _place_material_plans(self, mat_code: str, mat: Dict,
        ...
        for si in range(len(steps) - 1, -1, -1):
            ...
            if si == len(steps) - 1:
                ...
                upper_s = max(sh["shift_no"] for sh in self.shifts)
            else:
                nxt = step_slots.get(steps[si+1]["process_seq"], [])
                if nxt:
                    fd, fs, _, _ = min(nxt, key=lambda x: (x[0], x[1]))
